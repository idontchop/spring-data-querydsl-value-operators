...
public class QuerydslHttpRequestContextAwareServletFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        log.debug("Processing {}", ((HttpServletRequest) request).getRequestURI());
        ...
        try {
            ...
            if (optionalPath.isPresent()) {
                ...
                log.error(
                    "2No EntityPath found on requestURI: {}, bad filter configurations (check filter url pattern and also the injected mappings), filter is turning into a no-op for this request",
                    requestURI);
